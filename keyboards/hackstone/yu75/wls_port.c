...
void rgb_matrix_wls_indicator_set(uint32_t interval, uint8_t times) {
...
void wireless_devs_change_kb(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    switch (new_devs) {
        case DEVS_BT1: {
            if (reset) {
                rgb_matrix_wls_indicator_set(200, 1);
            } else {
                rgb_matrix_wls_indicator_set(500, 1);
            }
        } break;
        case DEVS_BT2: {
            if (reset) {
...
void timeout_task(void) {
    static uint32_t timeout_scan_timer = 0x00;
    if (timer_elapsed32(timeout_scan_timer) <= 50) {
        return;
    }
    timeout_scan_timer = timer_read32();
    // 无线模式灯光
    wls_rgb_blink();
}
