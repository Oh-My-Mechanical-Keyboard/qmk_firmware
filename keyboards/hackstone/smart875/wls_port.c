...
bool rgb_matrix_wls_indicator_cb(void) {
    IDEBUG_INFO("DEV:%d, ST: %d, TIM: %ld, EST: %ld", wireless_get_current_devs(), *md_getp_state(), wls_rgb_indicator_keep_timer, timer_elapsed32(wls_rgb_indicator_keep_timer));
    // 无线模式下没有连接正确持续闪
