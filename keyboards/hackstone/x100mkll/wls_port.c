...
enum wls_port_mio_mode wls_port_mio_scan(void) {
...
// 无线模式扫描50ms自动检测一次，update是否强制更新
void wls_port_mode_scan(bool update) {
    static uint32_t mode_scan_timer = 0x00;

    if ((update != true) && (timer_elapsed32(mode_scan_timer) <= (WLS_MODEIO_DETECTION_TIME))) {
        return;
    }
    mode_scan_timer = timer_read32();

    if (readPin(USB_CABLE_PIN)) {
        usb_cable_flag = true;
    } else {
        usb_cable_flag = false;
    }

    if (readPin(RF_MODE_SW_PIN) && !readPin(BT_MODE_SW_PIN)) {
        if ((wireless_get_current_devs() == DEVS_USB) || (wireless_get_current_devs() == DEVS_2G4)) {
            wireless_devs_change(wireless_get_current_devs(), confinfo.bt_devs, false);
        }
    }
    if (readPin(BT_MODE_SW_PIN) && !readPin(RF_MODE_SW_PIN)) {
...
bool rgb_power_is_allow_off(void) {
    ...
    if (usb_cable_flag) return false;
...
void wireless_devs_change_kb(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    switch (new_devs) {
        case DEVS_BT1: {
            if (reset) {
                rgb_matrix_wls_indicator_set(19, (RGB){.r = 0, .g = 0, .b = 0xff}, 200, 1);
            } else {
        ...
        case DEVS_BT2: {
            if (reset) {
                rgb_matrix_wls_indicator_set(20, (RGB){.r = 0, .g = 0, .b = 0xff}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(20, (RGB){.r = 0, .g = 0xff, .b = 0}, 500, 1);
            }
        } break;
        case DEVS_BT3: {
            if (reset) {
                rgb_matrix_wls_indicator_set(21, (RGB){.r = 0, .g = 0, .b = 0xff}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(21, (RGB){.r = 0, .g = 0xff, .b = 0}, 500, 1);
            }
        } break;
        case DEVS_2G4: {
            if (reset) {
                rgb_matrix_wls_indicator_set(22, (RGB){.r = 0, .g = 0, .b = 0xff}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(22, (RGB){.r = 0, .g = 0xff, .b = 0}, 500, 1);
            }
        } break;
        case DEVS_USB: {
            rgb_matrix_wls_indicator_set(23, (RGB){.r = 0, .g = 0xff, .b = 0}, 500, 1);
        } break;
        default:
            break;
    }
