...
void set_all_top_rgb_color(uint8_t r, uint8_t g, uint8_t b) {
...
void wireless_devs_change_kb(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    switch (new_devs) {
        case DEVS_BT1: {
            if (reset) {
                rgb_matrix_wls_indicator_set(0xFF, (RGB){.r=0, .g=0, .b=0xff}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(0xFF, (RGB){.r=0, .g=0xff, .b=0}, 500, 1);
            }
        } break;
        case DEVS_BT2: {
            if (reset) {
        ...
        case DEVS_USB: {
            rgb_matrix_wls_indicator_set(0xFF, (RGB){.r=0, .g=0, .b=0xff}, 500, 1);
        } break;
        default:
            break;
    }
...
void wls_rgb_blink(void) {
    if (wls_rgb_indicator_timer) {
        ...
        if (wls_rgb_indicator_times % 2) {
            set_all_top_rgb_color(wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
        } else {
            set_all_top_rgb_color(0x00, 0x00, 0x00);
        }
    } else {
        wls_rgb_indicator_keep_timer = 0;
    }
    if (wls_rgb_indicator_connect_timer != 0 && timer_elapsed32(wls_rgb_indicator_connect_timer) <= 2000) {
        ...
        case DEVS_2G4:
        case DEVS_USB: {
            set_all_top_rgb_color(0x00, 0xFF, 0x00);
        } break;
        }
    } else {
...
void wls_port_rgb_indicators_task(void) {
    ...
    if (inqbat_flag) {
        ...
        if (i < 10) {
            set_all_top_rgb_color(RGB_RED);
        } else if (i < 40) {
            set_all_top_rgb_color(RGB_ORANGE);
        } else if (i < 80) {
            set_all_top_rgb_color(RGB_CYAN);
        } else {
            set_all_top_rgb_color(RGB_GREEN);
        }
    } else {
        // 无线模式灯光
        wls_rgb_blink();
