...
#           endif
            ...
            waitInputPinDelay();
#ifndef LPWR_WAK_MODE
            palEnableLineEvent(row_pins[i], PAL_EVENT_MODE_BOTH_EDGES);
#else
            palEnableLineEvent(row_pins[i], LPWR_WAK_MODE);
#endif
        }
    }
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(row_pins); i++) {
        if (row_pins[i] != NO_PIN) {
            setPinOutputPushPull(row_pins[i]);
            writePin(row_pins[i], LPWR_OUTPUT_STATE);
        }
    }
...
#else
            palEnableLineEvent(col_pins[i], LPWR_WAK_MODE);
#endif
        }
    }
#endif
